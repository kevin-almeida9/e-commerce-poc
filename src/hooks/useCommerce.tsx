...
const CommerceContext = createContext<ICommerceContext>(INITIAL_COMMERCE_CONTEXT_VALUE);
...
export const CommerceProvider = ({children}: {children: ReactNode}) =>  {
  ...
  const getProductsList = useCallback(async () => {
    try {
      ...
      if (!Array.isArray(productResponse)) throw new Error('Não foi prossível listar os produtos, tente novamente mais tarde.')
  ...
  const addProductToCart = (product: IProduct) => {
    const alreadyHasProductIndex = cart.findIndex(item => item.id === product.id)

    if (alreadyHasProductIndex > 0) return cart[alreadyHasProductIndex].quantity++
  ...
  const removeProductToCart = (productId: number) => {
    ...
    if (removedProduct.quantity > 1) {
      setCart(prev => prev.map((item) => {
        if (item.id === productId) return { ...item, quantity: item.quantity--}
        return item
      }))
    }
